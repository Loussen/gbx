...
//	if($$Name=='') $error='Ad daxil edilməyib. (Dil: '.$lang_name.')';
	...
	if($error==''){
		...
		if($edit>0) mysqli_query($db,"update $do set $query_update where id='$edit' ");
		    ...
		    echo "insert into $do (position,active,$query_insert) values ('$position','$active',$query_insert_val) ";
		...
		if($uploadedFile!=''){
			makeThumb($imageFolder.'/'.$uploadedFile,$imageFolder.'/thumb_'.$uploadedFile,150,150);
		}
	}
}
