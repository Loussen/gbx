            ...
            <?php
                ...
                while($row=mysqli_fetch_assoc($sql))
                {
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="team-box mb-60" style="padding: 40px 20px 60px 20px;">
                            <div class="team-thumb mb-45 pos-rel">
                                <img style="border-radius: 5%; border: 1px solid #eee; padding: 5px;" src="<?=SITE_PATH?>/images/news/thumb_<?=$row['image']?>" alt="<?=$row['name_'.$lang_name]?>">
                            </div>
                            <div class="team-content">
                                <h3><a class="title-link" href="<?=$site?>/xeber/<?=slugGenerator($row['name_'.$lang_name]).'-'.$row["id"]?>"><?=$row['name_'.$lang_name];?></a></h3>
                                <p><?=substr_(decode_text($row['short_text_'.$lang_name]),0,100,true,true)?></p>
                                <a class="service-link float-right" href="<?=$site?>/xeber/<?=slugGenerator($row['name_'.$lang_name]).'-'.$row["id"]?>"><?=$lang1?></a>
                            </div>
                        </div>
                    </div>
                    <?php
